    ...
    public void Create()
    {
        ...
        for (int i = 0; i < levelLimit; i++)
        {
            ...
            if (isValueSeries)
            {
                ...
                data.value = valueCurve.Evaluate(tempFloat) * valueCurveMultiplier + valueStart;//Mathf.RoundToInt(valueCurve.Evaluate(tempFloat) * valueCurveMultiplier) + valueStart;
